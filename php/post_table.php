...
if(isset($_GET['ID']))
{
    ...
    if($result->num_rows >0)
    {
        ...
        while( $row=$result->fetch_assoc() )
        {
           echo' <tr>
            <td>'.$row['Post_ID'].'</td>
            <td class="w-25" style="word-break: break-all;">'.$row['Post_discription'].'</td>
            ...
            if (isset($row['Admin_ID']))
            {
              ...
              <input type ="button"  name="activate" disabled value="Activate " onclick="activate_post('.$row['Post_ID'].')"    class=" btn btn-success m-2 ">
              <input type ="button"  name="deactivate" disabled  value="Deactivate " onclick="deactivate_post('.$row['Post_ID'].')"     class=" btn btn-info m-2">
              <input type ="button"  name="deactivate" disabled  value="Remove " onclick="Remove_post('.$row['Post_ID'].')"     class=" btn btn-danger m-2">
              <input type ="button"  name="deactivate" disabled  value="Stop " onclick="Stop_post('.$row['Post_ID'].')"     class=" btn btn-primary m-2">
            ...
            echo  ' <td>
              ...
              <input type ="button"  name="deactivate"  value="Deactivate " onclick="deactivate_post('.$row['Post_ID'].')"     class=" btn btn-info m-2">
              <input type ="button"  name="deactivate"  value="Remove " onclick="Remove_post('.$row['Post_ID'].')"     class=" btn btn-danger m-2">
              <input type ="button"  name="deactivate"  value="Stop " onclick="Stop_post('.$row['Post_ID'].')"     class=" btn btn-primary m-2">
...
//for Stoping post valuation post here
else if(isset($_GET['stoppost_ID']))
{ $post_ID=$_GET['stoppost_ID'];
    $sql=" UPDATE post SET Post_status='Stop' WHERE Post_ID='$post_ID'";
    if ($conn->query($sql)===true)
    {
...
//for Remove post here
else if(isset($_GET['Removepost_ID']))
{ $post_ID=$_GET['Removepost_ID'];
    ...
    if($result_post->num_rows >0)
    {
      ...
      while( $row=$result_post->fetch_assoc())
      {
        ...
        $Average_pricing=null;
              ...
              $system_valuation=($Average_pricing-(($Average_pricing* $rate)/100));
